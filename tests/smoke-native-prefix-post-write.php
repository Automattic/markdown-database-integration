...
$checks = array(
	...
	'a prefix runtime created during a transaction leaves its live post write intact' => null === $begin->diagnostic() && 1 === $transient->return_value() && $during_transaction,
	'rollback restores canonical disk state and a cold provider read' => null === $rollback->diagnostic() && 0 === $after_rollback->wpdb_state()['num_rows'] && 0 === $cold_read->wpdb_state()['num_rows'] && array() === glob( $root . '/content/post/transient-title*' ),
	'commit preserves a split-root Markdown post write' => null === $commit_begin->diagnostic() && 1 === $commit_write->return_value() && null === $commit->diagnostic() && 'Committed title' === ( $commit_read->wpdb_state()['last_result'][0]->post_title ?? null ),
	'savepoint rewind restores the prior split-root Markdown post write' => null === $savepoint_begin->diagnostic() && 1 === $before_savepoint->return_value() && null === $savepoint->diagnostic() && 1 === $after_savepoint->return_value() && null === $rewind->diagnostic() && null === $savepoint_commit->diagnostic() && 'Before savepoint' === ( $savepoint_read->wpdb_state()['last_result'][0]->post_title ?? null ) && 'Before savepoint' === ( $cold_committed->wpdb_state()['last_result'][0]->post_title ?? null ),
);
