	...
	public function tokenize( string $sql ): array {
		...
		for ( $offset = 0; $offset < $length; ) {
			...
			if ( $this->is_identifier_start( $character ) ) {
				...
				$lexeme = substr( $sql, $start, $offset - $start );
				$type = in_array( strtoupper( $lexeme ), array( 'SELECT', 'DISTINCT', 'SQL_CALC_FOUND_ROWS', 'FROM', 'AS', 'INNER', 'LEFT', 'OUTER', 'JOIN', 'ON', 'WHERE', 'IN', 'LIKE', 'NOT', 'AND', 'OR', 'IS', 'NULL', 'BETWEEN', 'GROUP', 'ORDER', 'BY', 'ASC', 'DESC', 'LIMIT', 'CASE', 'WHEN', 'THEN', 'ELSE', 'END', 'SIGNED', 'UNSIGNED' ), true )
