<?php
/** Bounded and keyed reads share one request-scoped table snapshot. */
...
function mdi_native_bounded_remove_tree( string $root ): void {
	if ( ! is_dir( $root ) ) {
		return;
	}
...
function mdi_native_bounded_rows( WP_Markdown_Query_Result $result ): array {
	$rows = $result->wpdb_state()['last_result'] ?? array();
	return is_array( $rows ) ? array_map( static fn( $row ): array => (array) $row, $rows ) : array();
}

$root = sys_get_temp_dir() . '/mdi-native-bounded-' . bin2hex( random_bytes( 6 ) );
mkdir( $root, 0755 );
...
// Both query shapes read from the same loaded request snapshot.
$bounded = WP_Markdown_Native_Runtime_Factory::runtime( $root );
$bounded_rows = mdi_native_bounded_rows( $bounded->execute( new WP_Markdown_Query_Request( $query, 'wp_' ) ) );
$bounded_keyed = mdi_native_bounded_rows( $bounded->execute( new WP_Markdown_Query_Request( $keyed, 'wp_' ) ) );
...
$decoded_rows = mdi_native_bounded_rows( $decoded->execute( new WP_Markdown_Query_Request( $query, 'wp_' ) ) );
$decoded_keyed = mdi_native_bounded_rows( $decoded->execute( new WP_Markdown_Query_Request( $keyed, 'wp_' ) ) );
...
$reversed_rows = mdi_native_bounded_rows( $reversed->execute( new WP_Markdown_Query_Request( $query, 'wp_' ) ) );
...
$checks = array(
	'a bounded read returns the rows the bound asked for' => $expected === $bounded_rows,
	'a bounded read agrees with an already-loaded snapshot' => $decoded_rows === $bounded_rows,
	'a keyed read returns its row' => array( array( 'id' => '7', 'label' => 'row-07' ) ) === $bounded_keyed,
	'a keyed read agrees with an already-loaded snapshot' => $decoded_keyed === $bounded_keyed,
	'a file held out of order still reads in order' => $expected === $reversed_rows,
);

mdi_native_bounded_remove_tree( $root );
