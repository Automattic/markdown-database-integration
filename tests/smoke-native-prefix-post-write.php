...
$checks = array(
	...
	'a prefix runtime created during a transaction rejects Markdown writes before side effects' => null === $begin->diagnostic() && false === $transient->return_value() && ! $during_transaction,
	'rollback leaves rejected Markdown writes absent from warm and cold reads' => null === $rollback->diagnostic() && 0 === $after_rollback->wpdb_state()['num_rows'] && 0 === $cold_read->wpdb_state()['num_rows'] && array() === glob( $root . '/content/post/transient-title*' ),
	'commit leaves a transaction-rejected split-root Markdown write unchanged' => null === $commit_begin->diagnostic() && false === $commit_write->return_value() && null === $commit->diagnostic() && 'Updated title' === ( $commit_read->wpdb_state()['last_result'][0]->post_title ?? null ),
	'savepoint control leaves transaction-rejected split-root Markdown writes unchanged' => null === $savepoint_begin->diagnostic() && false === $before_savepoint->return_value() && null === $savepoint->diagnostic() && false === $after_savepoint->return_value() && null === $rewind->diagnostic() && null === $savepoint_commit->diagnostic() && 'Updated title' === ( $savepoint_read->wpdb_state()['last_result'][0]->post_title ?? null ) && 'Updated title' === ( $cold_committed->wpdb_state()['last_result'][0]->post_title ?? null ),
);
