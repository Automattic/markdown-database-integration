...
$verifier = new WP_Markdown_Native_Shadow_Verifier( $runtime, 3 );
...
$unsupported_query = "SELECT meta_value FROM wp_commentmeta WHERE meta_key = 'private@example.test' AND comment_id = 123";
...
$checks = array(
	'supported reads compare exactly without retaining observations' => 1 === $report['counts']['compatible']
	...
	'first unsupported query retains a sanitized reproducible shape' => 'unsupported' === ( $report['first_blocker']['status'] ?? null )
		&& hash( 'sha256', 'SELECT meta_value FROM wp_commentmeta WHERE meta_key = ? AND comment_id = ?' ) === ( $report['first_blocker']['query_template_sha256'] ?? null )
		...
		&& str_contains( (string) ( $report['first_blocker']['query_template'] ?? '' ), 'wp_commentmeta' ),
