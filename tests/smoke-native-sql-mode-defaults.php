...
try {
	...
	$aliased_count = $run( 'SELECT @@SESSION.warning_count AS `warning``count`' );
	$warnings_after_count = $run( 'SHOW WARNINGS' );
	...
	$checks['aliased warning count decodes quoted aliases without clearing warnings'] = '2' === $aliased_count->corpus_result()['rows'][0]['warning`count'] && 2 === $warnings_after_count->return_value();
	$set = $run( "SET SESSION sql_mode = 'strict_trans_tables,NO_ENGINE_SUBSTITUTION'" );
	$mode = $run( 'SELECT @@SESSION.sql_mode' );
	$aliased_mode = $run( 'SELECT @@SESSION.sql_mode `sql``mode`' );
	...
	$checks['bare sql mode alias decodes quoted identifier escapes'] = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' === $aliased_mode->corpus_result()['rows'][0]['sql`mode'];
	$checks['session aliases preserve case-insensitive AS and terminal semicolons'] = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' === ( $run( 'select @@session.sql_mode as mode;' )->corpus_result()['rows'][0]['mode'] ?? null );
	$checks['session variable trailing SQL fails closed'] = ! $run( 'SELECT @@warning_count AS warning_count FROM wp_defaults' )->succeeded();
	$checks['session variable aliases require an identifier rather than a bare keyword'] = ! $run( 'SELECT @@warning_count AS' )->succeeded() && ! $run( 'SELECT @@sql_mode FROM' )->succeeded();
