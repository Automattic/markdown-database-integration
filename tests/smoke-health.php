...
$mysql_full = WP_Markdown_Health::diagnose( array( 'mode' => 'primary', 'sqlite_runtime' => false, 'mysql_full_boundary' => true, 'mysql_full_sink' => true, 'mysql_full_outbox_diagnostics' => $outbox_diagnostics, 'backend_capabilities' => WP_Markdown_Backend_Capabilities::mysql_full() ) );
mdi_health_assert( 'mysql_full_outbox_ready' === $mysql_full['status'] && $mysql_full['healthy'] && $outbox_diagnostics === $mysql_full['outbox'], 'MySQL full-primary reports durable outbox readiness and backlog diagnostics without claiming canonical publication' );
